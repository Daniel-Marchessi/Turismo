	...
	{loop as $post}
		<div class="detail-half-content detail-portfolio-content">
				<div class="detail-thumbnail">
										...
										<div class="easy-slider-pager">
											{if $meta->type == 'video'}
												{if strpos($meta->videoUrl, 'youtube') !== false}
													...
													<a data-slide-index="0" href="#" title="{$post->title}">
														<span class="entry-thumbnail-icon">
															<img src="{imageUrl $post->imageUrl, width => 70, height => 70, crop => 1}" alt="{!$post->title}" class="detail-image">
														</span>
													</a>
												{/if}
											{else}
											...
											{foreach $pictures as $picture}
												...
												<a data-slide-index="{$iterator->getCounter()}" href="#" title="{$picture['title']}">
													<span class="entry-thumbnail-icon">
														<img src="{imageUrl $picture['image'], width => 70, height => 70, crop => 1}" alt="{!$picture['title']}" class="detail-image">
													</span>
												</a>
												{/if}
											{/foreach}
					...
					{else}
						{if $meta->type == 'video'}
							<div n:class='detail-thumbnail-wrap, detail-thumbnail-video, $meta->informations ? has-toggles'>
								<div class="loading"><span class="ait-preloader">{!__ 'Loading&hellip;'}</span></div>
								{if strpos($meta->videoUrl, 'youtube') !== false}
									<iframe src="{videoEmbedUrl $meta->videoUrl}" width="{$vidWidth}" height="{$vidHeight}" class="detail-youtube"></iframe>
								{else}
						...
						{elseif $meta->type == 'website'}
							<div n:class='detail-thumbnail-wrap, detail-thumbnail-website, entry-content, $meta->informations ? has-toggles'>
								{if $post->hasImage}
						...
						{else}
							<div n:class='detail-thumbnail-wrap, detail-thumbnail-image, entry-content, $meta->informations ? has-toggles'>
								{if $post->hasImage}
									<a href="{imageUrl $post->imageUrl, width => 1000, crop => 1}" class="thumb-link">
										<span class="entry-thumbnail-icon">
											<img src="{imageUrl $post->imageUrl, width => 1000, crop => 1}" alt="{!$post->title}" class="detail-image">
										</span>
									</a>
								{/if}
							</div>
