...
_aitSetCachePath($type){global$_aitPathsCache;$s='/cache/'.AIT_CURRENT_THEME;$dir=$_aitPathsCache['uploads']['basedir'].$s;$url=set_url_scheme($_aitPathsCache['uploads']['baseurl'].$s);if(!file_exists($dir)){wp_mkdir_p($dir);@copy(dirname(__FILE__).'/../../.htaccess',"$dir/.htaccess");}return$type=='dir'?$dir:$url;}function
...
aitPaths()->dir->fwConfig.'/plugins.php';self::$configuration['plugins']=apply_filters('ait-builtin-plugin-list',array_replace_recursive($pluginList,self::$configuration['plugins']));foreach(self::$configuration['plugins']as$slug=>$plugin){self::$configuration['plugins'][$slug]['slug']=$slug;if(isset($plugin['source'])and
...
function
tgmpa(){require_once
aitPaths()->dir->libs.'/class-tgm-plugin-activation.inc';if(is_admin()and!AitUtils::isAjax()){add_action('tgmpa_register',array(__CLASS__,'tgmpaRegisterPlugins'));}}static
function
tgmpaRegisterPlugins(){$config=array('domain'=>'ait-admin','parent_menu_slug'=>'plugins.php','parent_url_slug'=>'plugins.php','menu'=>'install-required-plugins','is_automatic'=>true,'strings'=>array('menu_title'=>__('Install Required Plugins','ait-admin')));if(!empty(self::$configuration['plugins'])){tgmpa(self::$configuration['plugins'],$config);}}static
function
...
get_locale();}static
function
getCurrentLanguageCode(){$locale=get_locale();if($locale=='zh_CN'){return'cn';}elseif($locale=='zh_TW'){return'tw';}elseif($locale=='pt_BR'){return'br';}else{return
...
self::_defaultLang();if(!is_admin()and!did_action('wp')){_doing_it_wrong(__METHOD__,'You can not get current language on frontend before WP Query (before "wp" action).  Action "template_redirect" is first safe place to use it.','1.0');return
self::_defaultLang();}$lang=null;if(function_exists('aitLangsGetCurrentLanguage')){$lang=aitLangsGetCurrentLanguage();}else{global$polylang;if(isset($polylang->curlang)and$polylang->curlang){$lang=$polylang->curlang;}else{self::_defaultLang();}}return$lang?$lang:self::_defaultLang();}static
function
...
function
getGmapsLang(){$map=array('bg_BG'=>'bg','cs_CZ'=>'cs','de_DE'=>'de','en_US'=>'en','es_ES'=>'es','fi'=>'fi','fr_FR'=>'fr','hi_IN'=>'hi','hr'=>'hr','hu_HU'=>'hu','id_ID'=>'id','it_IT'=>'it','nl_NL'=>'nl','pl_PL'=>'pl','pt_BR'=>'pt-BR','pt_PT'=>'pt-PT','ru_RU'=>'ru','sk_SK'=>'sk','sq'=>'sq','sv_SE'=>'sv','zh_CN'=>'zh-CN','zh_TW'=>'zh-TW');$currentLocale=self::getCurrentLocale();if(isset($map[$currentLocale])){return$map[$currentLocale];}return'en';}}class
AitAdminBar{static
function
...
as$i=>$elementsFullConfig){$elementId=$elementsFullConfig['@element'];$element=$this->createPrototype($elementId,$elementsFullConfig,$defaults[$i][$elementId]);$this->prototypes[$elementId]=$element;}}function
createPrototype($elementId,$fullConfig,$defaults){$className='AitElement';$class=AitUtils::id2class($elementId,'Element');$classfile=aitPath('elements',"/{$elementId}/{$class}.php");if($classfile!==false){$className=$class;}if(isset($fullConfig['@configuration']['class'])and!empty($fullConfig['@configuration']['class'])){$className=$fullConfig['@configuration']['class'];}return
new$className($elementId,$fullConfig,$defaults);}function
...
AitLessFormatterCompressed;$less->setPreserveComments(false);$less->setFormatter($formatter);}self::$less[$hash]=$less;}return
...
array();}}function
getOptions($oid=null){if($oid){if(isset($this->storage["local$oid"]))return$this->storage["local$oid"];$this->storage["local$oid"]=$this->getLocalOptions($oid);return$this->storage["local$oid"];}else{if(isset($this->storage['global']))return$this->storage['global'];$this->storage['global']=$this->getGlobalOptions();return$this->storage['global'];}}protected
function
...
and!$fullConfig[$i]['@used']){unset($defaultsOfUsedElements[$i]);}}$this->storage["getDefaultsOnlyOfUsedElements"]=$defaultsOfUsedElements;return$this->storage["getDefaultsOnlyOfUsedElements"];}protected
function
...
and
self::$frontpage->blog){$blogPage=get_page(self::$frontpage->blog)->post_title;}$pages=array('_blog'=>array('label'=>$blogPage?sprintf(__('%s (blog)','ait-admin'),$blogPage):__('Homepage (blog)','ait-admin'),'with-id'=>false,'if'=>'is_home()'),'_404'=>array('label'=>__('404 page','ait-admin'),'with-id'=>false,'if'=>'is_404()'),'_search'=>array('label'=>__('Search page','ait-admin'),'with-id'=>false,'if'=>'is_search()'),'_archive'=>array('label'=>__('Archive pages','ait-admin'),'sub-label'=>__('Category, Taxonomy, Tag, Author, Date','ait-admin'),'with-id'=>false,'if'=>'is_archive()'),'_attachment'=>array('label'=>__('Attachment pages','ait-admin'),'with-id'=>false,'if'=>"is_attachment()"),'_post'=>array('label'=>__('Single post','ait-admin'),'with-id'=>false,'if'=>"is_singular('post')"));if(aitIsPluginActive('toolkit')){$cpts=aitManager('cpts')->getAll();foreach($cpts
as$cpt){if($cpt
instanceof
AitPublicCpt){$pages["_{$cpt->getId()}"]=array('label'=>$cpt->getLabels()->singular_name,'with-id'=>false,'if'=>"is_singular('{$cpt->getInternalId()}')");}}}if(AitWoocommerce::enabled()){$pages['_wc_product']=array('label'=>__('Single - WooCommerce Product','ait-admin'),'with-id'=>false,'if'=>"AitWoocommerce::currentPageIs('product')");$shop=AitWoocommerce::getPage('shop');if($shop){$pages['_wc_shop']=array('label'=>sprintf(__('%s - WooCommerce Shop Page','ait-admin'),$shop->post_title),'with-id'=>false,'if'=>"AitWoocommerce::currentPageIs('woocommerce')");}}self::$specialPages=apply_filters('ait-special-custom-pages',$pages);return
...
extends
NObject{const
UNDEFINED_INDEX=1000;protected$internalId;protected$id;protected$title;protected$instanceNumber=0;protected$config;protected$configuration=array();protected$optionsDefaults=array();protected$options=array();protected$optionsControlsGroup;protected$oid='';protected$cpts=array();protected$template;protected$templateName;protected$style;protected$styleLessName;protected$assets=array('js'=>array(),'css'=>array(),'admin-js'=>array(),'admin-css'=>array());protected$paths;protected$generatedCss=array();protected$lessParser;protected$optionsObject;protected$isBetweenSidebars=false;protected$used=false;function
...
static$themeVersionOptionKey;protected$errors=array();static
function
run(){add_action('admin_init',array(__CLASS__,'maybeDoUpgradeOnAdminInit'));add_action('ait-after-import',array(__CLASS__,'maybeDoUpgradeAfterImport'),10,2);add_action('ait-theme-activation',array(__CLASS__,'addVersionsOnThemeActivation'));}static
function
maybeDoUpgradeOnAdminInit(){if(AitUtils::isAjax()or
defined('IFRAME_REQUEST'))return;$upgrader=new
self;$upgrader->maybeDoUpgrade();}static
function
addVersionsOnThemeActivation(){if(get_option(self::$skeletonVersionOptionKey,'')===''){add_option(self::$skeletonVersionOptionKey,AIT_SKELETON_VERSION);}if(get_option(self::$themeVersionOptionKey,'')===''){add_option(self::$themeVersionOptionKey,AIT_THEME_VERSION);}}function
__construct(){if(get_option("_ait_".AIT_CURRENT_THEME."_skeleton_version")===false){self::$skeletonVersionOptionKey="_ait_skeleton_version";self::$themeVersionOptionKey="_ait_theme_version";}else{self::$skeletonVersionOptionKey="_ait_".AIT_CURRENT_THEME."_skeleton_version";self::$themeVersionOptionKey="_ait_".AIT_CURRENT_THEME."_theme_version";}}function
maybeDoUpgrade(){if(version_compare(self::getSkeletonVersion(),AIT_SKELETON_VERSION,'<')){$this->skeletonUpgrade();if($this->noErrors()){$this->updateSkeletonVersionToNewest();AitCache::clean();}}if(version_compare($this->getThemeVersion(),AIT_THEME_VERSION,'<')){$this->themeUpgrade();if($this->noErrors()){$this->updateThemeVersionToNewest();AitCache::clean();}}}function
...
updateThemeVersionToNewest(){update_option(self::$themeVersionOptionKey,AIT_THEME_VERSION);}}class
AitOptionControl
extends
...
control(){$val=$this->getValue();if(is_null($val))$val=0;?>
			...
			<div class="ait-opt ait-enable-advanced">
				...
				<select id="<?php echo$this->getIdAttr()?>" name="<?php echo$this->getNameAttr()?>" class="ait-toggle-advanced">
					<option value="1" <?php selected(1,$val)?>><?php _e('Yes','ait-amdin')?></option>
					<option value="0" <?php selected(0,$val)?>><?php _e('No','ait-amdin')?></option>
				</select>
				</div>
			</div>
			<div class="ait-opt-help">
